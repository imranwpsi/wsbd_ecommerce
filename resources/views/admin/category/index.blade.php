...
<div class="col-lg-12">
    <div class="main-card mb-3 card">
        <div class="card-body">
        	...
        	<button type="button" class="btn mr-2 mb-2 btn-primary float-right addModalBtn" url="{{ route('category.create') }}">
        	    {{trans('label.create_category')}}
        	</button>
            <table class="mb-0 table table-hover table-bordered">
                <thead>
                <tr>
                    <th>{{ trans('label.name') }}</th>
                    <th class="text-center">{{ trans('label.banner') }}</th>
                    <th class="text-center">{{ trans('label.icon') }}</th>
                    <th class="text-center">{{ trans('label.featured') }}</th>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @if($category->banner)
                        <img width='30' src="{{ asset($category->banner)}}"/>
                        @else
                    ...
                    <td class="text-center">
	                    @if($category->icon)
	                    <img width='30' src="{{ asset($category->icon)}}"/>
	                    @else
	                    <img width='30' src="{!! asset('avatar.jpg')!!}">
	                    @endif
                	</td>
                    ...
                    <td class="text-center">
					<button class="mb-2 mr-2 btn-icon btn-icon-only btn btn-warning btn-xs editModalBtn" url="{{ route('category.edit',$category->id) }}"><i class="lnr-pencil btn-icon-wrapper"></i></button>
